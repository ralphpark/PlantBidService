  ...
  const renderMessages = () => {
    ...
    chatMessages.forEach(msg => {
      if (msg.role === 'vendor') {
        ...
        if (msg.content) {
          // 정확히 일치하는 상태 메시지 체크
          const isExactStatusMessage = statusMessages.has(msg.content);
          // 상품명이 포함된 동적 메시지 체크 (예: "상품이 추가되어 입찰을 완료중입니다. 상품명: ...")
          const isDynamicStatusMessage = msg.content.includes('검토') || 
                                          msg.content.includes('완료중') || 
                                          msg.content.includes('입찰이 완료되었습니다');
          
          if (isExactStatusMessage || isDynamicStatusMessage) {
            // 메시지 타입별로 한 번만 표시 (예: 검토중 메시지는 한 번만)
            let messageType;
            if (msg.content.includes('검토')) {
              messageType = 'reviewing';
            } else if (msg.content.includes('완료중')) {
              messageType = 'in_progress';
            } else if (msg.content.includes('입찰이 완료되었습니다')) {
              messageType = 'completed';
            } else {
              messageType = msg.content;
            }
            
            if (seenStatusMessagesPerVendor[vendorId].has(messageType)) {
              // 이미 같은 타입의 상태 메시지가 있으면 스킵
              return;
            }
            seenStatusMessagesPerVendor[vendorId].add(messageType);
          }
        }
      }
    ...
    return processedMessages.map((msg, index) => {
      ...
      // 판매자 메시지이고 제품 정보가 있는 경우
      if (isVendor && msg.product) {
        // 검토중 상태일 때는 제품 카드를 숨기고 상태 메시지만 표시
        if (msg.bidStatus === 'reviewing' || msg.bidStatus === 'sent') {
          // 상태 메시지가 있으면 표시, 없으면 스킵
          const isStatusMessage = msg.content && (
            statusMessages.has(msg.content) || 
            msg.content.includes('검토') || 
            msg.content.includes('완료중')
          );
          ...
          return null; // 검토중 상태에서 상태 메시지가 없으면 아무것도 표시 안 함
        }
        ...
        return (
          <div key={`msg-${index}`} className={`my-3 rounded-lg p-4 ${vendorColorClass} shadow-sm`}>
            ...
            {msg.content && msg.content.trim() !== '' && (
              (() => {
                const isStatusMsg = statusMessages.has(msg.content) || 
                                   msg.content.includes('검토') || 
                                   msg.content.includes('완료중') ||
                                   msg.content.includes('입찰이 완료되었습니다');
                
                if (isStatusMsg) return null; // 상태 메시지는 별도로 표시됨
        ...
        >
          <div className="flex items-center mb-2">
            <div className="font-semibold">
              {isUser ? '고객' : 
               isAssistant ? 'AI 상담사' :
               isVendor ? (msg.vendorName || msg.storeName || '판매자') :
               '알 수 없음'} 메시지:
            </div>
            <div className="ml-auto text-xs text-gray-500">
              {new Date(msg.timestamp).toLocaleString('ko-KR', {
